...
async def update_agent_run_status(db: AsyncSession, run_id: int, status: str, state: dict = None) -> AgentRun:
...
# --- Approval Helpers ---
async def create_approval(db: AsyncSession, agent_run_id: int, action_type: str, action_details: dict) -> Approval:
    approval = Approval(agent_run_id=agent_run_id, action_type=action_type, action_details=action_details, status="pending")
    db.add(approval)
    await db.commit()
    await db.refresh(approval)
    return approval

async def respond_to_approval(db: AsyncSession, approval_id: int, approve: bool) -> Approval:
