...
def run_research_crew(query: str) -> dict:
    ...
    if not (has_groq or has_cerebras) or not has_tavily:
        print(
            "WARNING: Neither GROQ_API_KEY nor CEREBRAS_API_KEY is configured (or TAVILY_API_KEY is missing). Returning mock research results for testing."
        )
    ...
    try:
        ...
        planner_output = (
            plan_task.output.raw if (hasattr(plan_task, "output") and plan_task.output) else ""
        )
        crawler_output = (
            crawl_task.output.raw if (hasattr(crawl_task, "output") and crawl_task.output) else ""
        )
        analyst_output = (
            analyse_task.output.raw
            if (hasattr(analyse_task, "output") and analyse_task.output)
            else ""
        )
        writer_output = (
        ...
        if planner_output:
            ...
            if not sub_questions:
                sub_questions = [
                    line for line in planner_output.splitlines() if line.strip() and len(line) > 10
                ]
        ...
        if analyst_output:
            ...
            if confidence == 0.8:
                matches = re.findall(
                    r"(?:confidence|score|rating)(?:\s*\w*){0,3}\s*[:=-]?\s*(0\.\d+|1\.0|1)",
                    analyst_output,
                    re.IGNORECASE,
                )
                if matches:
        ...
        print(
            f"--- CrewAI execution finished successfully. Confidence={confidence}, Sources={len(sources)} ---"
        )
        return {
            "briefing": briefing,
            "sources": sources,
        ...
        print(
            f"WARNING: CrewAI sub-crew execution failed: {e}. Falling back to mock research results."
        )
        return get_mock_results(query)
